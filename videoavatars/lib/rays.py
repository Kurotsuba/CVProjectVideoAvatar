...
def unpose_and_select_rays(rays, Vi, smpl, rn_b, rn_m):
    ...
    valid_rays = dist[tuple(np.vstack((ray_matches, range(dist.shape[1]))).tolist())] < 0.12
    valid_verts = dist[tuple(np.vstack((range(dist.shape[0]), vert_matches)).tolist())] < 0.03
